...
import { Coin, CoinDocument } from '../../schema/Coin/coin.schema';
import {
  CoinPrice,
  CoinPriceDocument,
...
import { FeeInfo, FeeInfoDocument } from '../../schema/FeeInfo/fee-info.schema';
import { Network, NetworkDocument } from '../../schema/Network/network.schema';
import {
  StakeInvestmentInfo,
  StakeInvestmentInfoDocument,
...
import { Wallet, WalletDocument } from '../../schema/Wallet/wallet.schema';
import { dbCoins } from '../../utils/utils';
...
export class CoinsService {
  constructor(
    ...
    @InjectModel(StakeInvestmentInfo.name)
    private _stakeInvestmentInfoModel: Model<StakeInvestmentInfoDocument>,
        ...
        for (const dbCoin of dbCoins) {
...

          {
            if (dbCoin?.isStakingAvailable) {
              const stakingService = new StakeService(
                dbCoin?.stakingContractAddress,
                networkDocument?.rpcUrl,
                dbCoin?.contractAddress,
              );

              const stakingPlans = await stakingService.getStakingPlans();
              debugger;
              await Promise.all(
                stakingPlans.map(async (stakingPlan) => {
  ...
  async updatePriceFromCoinGeckoTerminal(coin: CoinDocument) {
    try {
      ...
      const url = `${
        process.env.COINGECKO_TERMINAL_API_URL
      }simple/networks/${networkData?.symbol?.toLowerCase()}/token_price/${
        coin?.coinNameId
      }`;
      ...
      const usdPrice = Number(
        data?.data?.attributes?.token_prices?.[coin?.coinNameId],
      );

      const usdtCoin = await this._coinModel.findOne({ coinNameId: 'tether' });

      const usdPricesInAllCurrencies = await this._coinPriceModel.find({
        coinId: usdtCoin?.id,
      });
      ...
      const coinPriceInAllCurrencies = currencies.map((currency) => {
        const usdPriceInCurrency = usdPricesInAllCurrencies.find(
          (usdPriceInCurrency) =>
            usdPriceInCurrency?.currencyId == currency?.id,
        )?.price;
  ...
  async updatePrice(coin: CoinDocument, currencies: CurrencyDocument[]) {
    try {
      ...
      if (coin?.priceFrom === 'COINGECKO') {
        ...
        const usdtCoin = await this._coinModel.findOne({
          coinNameId: 'tether',
        });
  ...
  async updatePriceForAllCurrencies() {
    try {
      ...
      await Promise.all(
        allCoins
          ?.filter((item) => item?.priceFrom === 'COINGECKO')
          .map(async (coin) => {
      ...
      for await (const coin of allCoins?.filter(
        (item) => item?.priceFrom === 'COINGECKO_TERMINAL',
      )) {
  ...
  @Cron('0 */30 * * * *')
  async updateStakingInfoForAllCoins(coinIds: string[] = []) {
    try {
      const query = {};
      if (coinIds.length) {
  ...
  async removeDuplicateCurrencies() {
    try {
      ...
      for await (const currency of allCurrencies) {
        const duplicateCurrencies = await this._currencyModel.find({
          name: currency.name,
        });
        if (duplicateCurrencies.length > 1) {
          await Promise.all(
            duplicateCurrencies.slice(1).map(async (duplicateCurrency) => {
              // update user currency to the first currency
              const users = await this._userModel.find({
                currencyId: duplicateCurrency?.id,
              });

              await Promise.all(
                users.map(async (user) => {
                  await user.updateOne({
                    currencyId: duplicateCurrencies?.[0].id,
                  });
                  console.log(
                    'User currency updated',
                    user?.email,
                    duplicateCurrencies?.[0]?.name,
                  );
                }),
              );

              await this._currencyModel.deleteOne({
                _id: duplicateCurrency.id,
              });
  ...
  async getUserInvestmentData(user) {
    try {
      ...
      for await (const coin of coins) {
        ...
        const userStakingData = await stakingService?.getUserStakingInformation(
  ...
  async getAllInvestmentDataForInvestmentIds(
    investmentIds: number[] = [],
    coinId: string,
  ) {
    try {
      const coin = await this._coinModel
        .aggregate([
          {
            $match: {
              _id: coinId,
      ...
      const userStakingData = await stakingService?.getStakingInformations(
        investmentIds,
      );

      const uniqueWallets = [
        ...new Set(userStakingData.map((item) => item?.investor)),
      ];

      const wallets = await this._walletModel
        .find({
      ...
      const userStakingDataWithUserId = userStakingData.map((item) => {
        const wallet = wallets?.find(
          (wallet) => wallet?.evmAddress === item?.investor,
        );
      ...
      debugger;
      const batch =
        this._stakeInvestmentInfoModel.collection.initializeUnorderedBulkOp();
      debugger;
        ...
        batch
          ...
          .updateOne({
            $set: {
              ...
              stakingContractAddress:
                data?.stakingContractAddress?.toLowerCase(),
              investmentId: Number(data?.id),
  ...
  async getAllInvestmentData(startInvestmentIndex: number = 1) {
    try {
      ...
      for await (const coin of coins) {
        ...
        let userStakingData = await stakingService?.getAllStakingInformation(
          startInvestmentIndex,
        );
